...
export class UserService extends APIService {
  ...
  async savePushSubscription(subscription: {endpoint?: string; auth_key: string,p256dh_key: string}): Promise<any> {
    return this.post("/api/workspaces/browser/notificatiuons/", subscription)
      .then((response) => response?.data)
      .catch((error) => {
        throw error?.response?.data;
      });
  ...
  async updatePushSubscription(subscription: { allow: boolean }): Promise<any> {
    return this.patch("/api/workspaces/browser/notifications/toggle-enable/", {
      is_enable: subscription.allow,
    })
      .then((response) => response?.data)
      .catch((error) => {
        throw error?.response?.data;
  ...
  async getPushSubscription(): Promise<any> {
    return this.get("/api/workspaces/browser/notifications/latest/")
      .then((response) => response?.data)
      .catch((error) => {
        // Optionally return null if 404 (no subscription yet)
        if (error?.response?.status === 404) return null;
        throw error?.response?.data;
      });
  }
  
  async deletePushSubscription(subscription: { endpoint?: string }): Promise<any> {
    return this.delete("/api/workspaces/browser/notifications/delete/", {
  ...
  async getUserProfileData(workspaceSlug: string, userId: string, params?: { time_range?: string }): Promise<IUserProfileData> {
    return this.get(`/api/workspaces/${workspaceSlug}/user-stats/${userId}/`, { params })
      .then((response) => response?.data)
      .catch((error) => {
        throw error?.response?.data;
      });
  ...
  async deleteAccount(email: any) {
    return this.delete(`/api/users/me/delete-account/`, {email: email})
      .then((response) => response?.data)
      .catch((error) => {
        throw error?.response?.data;
      });
  ...
  async getSalesStats(workspaceSlug: string, userId: string): Promise<any> {
  return this.get(`/api/workspaces/${workspaceSlug}/sales-user-stats/${userId}/`)
    .then((response) => response?.data)
    .catch((error) => {
      throw error?.response?.data;
    });
